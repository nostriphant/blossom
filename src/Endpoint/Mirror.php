<?php

namespace nostriphant\Blossom\Endpoint;

readonly class Mirror implements \nostriphant\Blossom\Endpoint {
    
    private Factory $factory;
    
    public function __construct(\nostriphant\Blossom\Blob\Factory $blob_factory, \nostriphant\NIP01\Key $server_key, \nostriphant\Blossom\UploadConstraints $upload_authorized) {
        $this->factory = new Factory(fn(\nostriphant\HTTP\ServerRequest $request) => [$upload_authorized, $blob_factory(null), $server_key, $request]);
    }
    
    #[\Override]
    public function __invoke(callable $define): void {
        $define(\nostriphant\HTTP\Method::PUT, true, ($this->factory)(Mirror\Put::class));
    }
}
