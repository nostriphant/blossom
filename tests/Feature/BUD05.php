...
describe("OPTIONS /media", function() {
    it('For preflight (OPTIONS) requests, servers MUST also set, at minimum, the Access-Control-Allow-Headers: Authorization, * and Access-Control-Allow-Methods: PUT headers.', function () {
        ...
        $response = FeatureCase::request('OPTIONS', '/media');
...
it('The PUT /media endpoint MUST accept binary data in the body of the request', function (string $contents, string $hash) {
    ...
    $response = FeatureCase::request('PUT', '/media', upload_resource: tmpfile(), authorization:['t' => 'upload', 'x' => $hash, 'key' => '6eeb5ad99e47115467d096e07c1c9b8b41768ab53465703f78017204adc5b0cc']);
    ...
    $response = FeatureCase::request('PUT', '/media', upload_resource: $resource, authorization:['t' => 'upload', 'x' => $hash]);
    ...
    expect($blob_descriptor->size)->toBe(14, $response->body);
...
it('The PUT /media endpoint MUST honor upload size limit', function () {
...
it('The PUT /media endpoint MUST check content-length when existing', function () {
    ...
    $response = FeatureCase::request('PUT', '/media', upload_resource: $resource, authorization:['t' => 'upload', 'x' => $hash], headers:[
...
it('The PUT /media endpoint MUST check content-type when existing', function () {
    ...
    expect($response->headers['x-reason'])->toBe('Unsupported file type "audio/wav".');
...
it('Servers MUST accept DELETE requests to the /<sha256> endpoint', function () {
    ...
    $response = FeatureCase::request('GET', '/' . $blob_descriptor->sha256, authorization:['t' => 'get', 'x' => $hash]);
    expect($response->status)->toBe('200');
    expect($response->headers['access-control-allow-origin'])->toBe('*');
    
    $response = FeatureCase::request('DELETE', '/' . $blob_descriptor->sha256, authorization:['t' => 'delete', 'x' => $hash, 'key' => '6eeb5ad99e47115467d096e07c1c9b8b41768ab53465703f78017204adc5b0cc']);
    expect($response->status)->toBe('403');
    
    $response = FeatureCase::request('DELETE', '/' . $blob_descriptor->sha256, authorization:['t' => 'delete', 'x' => $hash]);
    expect($response->status)->toBe('204');
    expect($response->headers['access-control-allow-origin'])->toBe('*');
    ...
    $response = FeatureCase::request('GET', '/' . $blob_descriptor->sha256, authorization:['t' => 'get', 'x' => $hash]);
    expect($response->status)->toBe('404', $response->headers['x-reason'] ?? $response->body);
    expect($response->headers['access-control-allow-origin'])->toBe('*');
