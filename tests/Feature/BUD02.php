...
it('The PUT /upload endpoint MUST accept binary data in the body of the request', function (string $contents, string $hash) {
    ...
    $response = FeatureCase::request('GET', '/list/15b7c080c36d1823acc5b27b155edbf35558ef15665a6e003144700fc8efdb4f', upload_resource: $resource, authorization:['t' => 'upload', 'x' => $hash]);
    expect($response->status)->toBe('200');
    
    
    $response = FeatureCase::request('PUT', '/upload', upload_resource: tmpfile(), authorization:['t' => 'upload', 'x' => $hash, 'key' => '6eeb5ad99e47115467d096e07c1c9b8b41768ab53465703f78017204adc5b0cc']);
    ...
    $response = FeatureCase::request('PUT', '/upload', upload_resource: $resource, authorization:['t' => 'upload', 'x' => $hash]);
    ...
    expect($blob_descriptors)->not()->toBeNull($response->body);
    ...
    expect($blob_descriptors[0]->size)->toBe(14, $response->body);
